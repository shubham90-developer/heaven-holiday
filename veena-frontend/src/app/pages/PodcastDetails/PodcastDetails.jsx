...
const PodcastAllList = () => {
  ...
  const podcastDetail = data?.data?.find(
    (item) => item._id === podcastId
  );

  if (!podcastDetail) return <p>Podcast not found</p>;

  const filteredEpisodes = podcastDetail?.episodesList?.filter((ep) =>
    ep.title.toLowerCase().includes(search.toLowerCase())
  );

  const handlePlay = (audioUrl, episodeId) => {
    if (!audioUrl) return;

    // If same episode clicked → toggle
            ...
            >
              <div className="relative">
                ...
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    handlePlay(ep.audioUrl, ep._id);
                  }}
                  className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white text-xs rounded-md"
                >
                  {currentPlaying === ep._id ? "II" : "▶"}
                </button>
              </div>
